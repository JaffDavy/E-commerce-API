...
export const addProduct = async (req, res) => {
  try {
    ...
    if (!name || !price) {
      logger.warn(
        `Attempt to add product with missing fields: ${JSON.stringify(req.body)}`,
      );
...
export const getProduct = async (req, res) => {
  try {
    ...
    logger.error(
      `Failed to fetch product ID ${req.params.id}: ${error.message}`,
    );
    res.status(500).json({ error: error.message });
  }
};
...
export const updateProduct = async (req, res) => {
  try {
    ...
    logger.error(
      `Failed to update product ID ${req.params.id}: ${error.message}`,
    );
    res.status(500).json({ error: error.message });
  }
};
...
export const deleteProduct = async (req, res) => {
  try {
    ...
    logger.error(
      `Failed to delete product ID ${req.params.id}: ${error.message}`,
    );
    res.status(500).json({ error: error.message });
  }
};
